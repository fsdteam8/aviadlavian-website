...
const ResultDetails = () => {
  ...
  const { data, isLoading } = useQuery<{ success: boolean; data: ResultData }>({
    queryKey: ["result-details", resultId],
    queryFn: async () => {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/examattempt/${id}/questions/${resultId}/result`,
        {
  ...
  return (
    <div className=" p-6 bg-white font-sans text-slate-900">
      ...
      <div className="space-y-3 mb-10">
        {options.map((option) => {
          const isUserSelected = option._id === selectedOptionId;
